...
def _pack_vertical(arr: list, margin=None, spacing=None, width=None, height=None,
...
def _pack_horizontal(arr: list, margin=None, spacing=None, width=None, height=None, align=None, flex: int = None,
...
def _pack_baseline(arr: list, margin=None, spacing=None, width=None, height=None, paddingAll=None,
...
# textを作成する関数
def _pack_text(text: str, color=None, size=None, flex=None, uri=None, weight=None, margin=None, decoration=None):
...
# imageを作成する関数
def _pack_image(path, size=None, aspectRatio=None, aspectMode=None):
...
# iconを作成する関数
def _pack_icon(path, size, scaling=None):
...
# separetorを作成する関数
def _pack_separator(margin="none"):
...
# 円形をつくる関数
def _pack_circle(width, hegiht, cornerRadius="30px", borderColor="#ff0000", borderWidth="2px"):
	return _pack_vertical([
		_pack_vertical(
			[],
...
def _create_message_schedule_plan(title: str, start_time: datetime.datetime, end_time: datetime.datetime, color=None):
	message_arr = []
	...
	if start_time.hour > 7:
		message_arr.append(
			_pack_vertical(arr=[], flex=7 - start_time.hour),
		)

	# 予定の追加
	message_arr.append(
		_pack_vertical(arr=[
			_pack_text(text=title, size="xxs"),
			_pack_text(text="S: " + start_time_str, size="xxs"),
			_pack_text(text="E: " + end_time_str, size="xxs")
	...
	if end_time.hour < 22:
		message_arr.append(
			_pack_vertical(arr=[], flex=end_time.hour - 22)
		)

	message = _pack_horizontal(arr=message_arr)

	return message


def _get_icon(icon_kind, icon_file_kind):
	icon_file_path = ""
...
def _package_event_all_body(events: list):
	...
	title_box = _pack_horizontal([_pack_text("終日", size="lg", weight="bold")])
	temp_event = []
	...
	_message_body_all_day = _pack_vertical([title_box, event_detail])
...
def _package_event_schedule(events: list):
	...
	# テンプレート
	title_box = _pack_horizontal([_pack_text("スケジュール", size="lg", weight="bold")])
	temp_event = []
	...
	for event in events:
		...
		# アイコンの取得
		icon_path = _get_icon(icon_kind="event", icon_file_kind=event_kind)
	...
	# メッセージに格納する
	_message_body_schedule = _pack_vertical([title_box, event_detail], paddingTop="md")
...
def _package_body(events: list):
	...
	if not events["all_day_events"] and not events["schedule_events"]:
		_body = _pack_vertical([_pack_text("予定なし", color="#0000a0", size="xl", weight="bold")], margin="lg",
	...
	# どちらも
		...
		body_event_list = [
			_message_body_all_day,
			_pack_separator(margin="lg"),
			_message_body_schedule
		]

	_message_body = _pack_vertical(body_event_list, paddingAll="lg")
...
def _package_footer():
	_message_footer = _pack_vertical(
		[_pack_text("\"Google Calendar\" を開く", color="#0000ff", uri=FOOTER_URL, decoration="underline")])
...
def package_message_one_day(events_list: dict):
	...
	logger.debug(f'payload: {_message}')
...
def package_carousel_message(schedule_dict: dict):
	...
	for schedule_list_index in range(len_date):
		...
		payload = package_message_one_day_none_image(date=schedule_date,
